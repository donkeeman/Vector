...
test("paused 상태에서 !start를 보내도 라우터는 추가 답장 없이 start 의미만 전달한다", async () => {
  const calls = [];
...
test("루트 DM의 자기소개와 사용법 질문은 키워드 기반 고정문구로 답하고 LLM을 호출하지 않는다", async () => {
  ...
  assert.deepEqual(slackClient.replies, [
    {
      threadTs: "1000.21",
      text: "나? 네가 CS 공부하다 절망에 빠지는 순간을 실시간으로 감상하러 온 천재, 벡터 라고 해. 긴장 좀 하는 게 좋을걸?",
    },
    {
      threadTs: "1000.215",
      text: "나? 네가 CS 공부하다 절망에 빠지는 순간을 실시간으로 감상하러 온 천재, 벡터 라고 해. 긴장 좀 하는 게 좋을걸?",
    ...
    {
      threadTs: "1000.22",
      text: "별거 없어. 덤빌 준비됐으면 `!start` 치고, 도저히 못 버티겠으면 `!stop` 치고 도망가든가. 도움말이 필요하면 `!help`나 쳐. 간단하지?",
    },
    {
      threadTs: "1000.221",
      text: "별거 없어. 덤빌 준비됐으면 `!start` 치고, 도저히 못 버티겠으면 `!stop` 치고 도망가든가. 도움말이 필요하면 `!help`나 쳐. 간단하지?",
